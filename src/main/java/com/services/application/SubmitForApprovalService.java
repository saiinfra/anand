	...
	public void delPkgInBase(PackageInformationDO pkgInfoDO,
		...
		if (pkgList.size() > 0) {
			for (Iterator iterator = pkgList.iterator(); iterator.hasNext();) {
				...
				List<Object> releasePackageList = releasePackageDAO
						.findByPkgIDAndRID(ids[0],
								pkgInfoDO.getReleaseInformationId(), sfhandle);
				
				for (Iterator iterator2 = releasePackageList.iterator(); iterator2
						.hasNext();) {
					ReleaseInformationDO object = (ReleaseInformationDO) iterator2.next();

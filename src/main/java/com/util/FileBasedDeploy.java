...
public class FileBasedDeploy {
	...
	public void deploy(String bOrgId, String bOrgToken, String bOrgURL,
			...
			boolean isValidate, MetadataLogDO metadataLogDO) throws Exception {
		...
		if (!result.isSuccess()) {
			...
			if (isValidate) {
				System.out.println("Validation Success ID"
						+ asyncResult.getId());
				metadataLogDO.setValidationSuccessId(asyncResult.getId());
				RDAppService.updateMetadataLogValidationID(metadataLogDO,
